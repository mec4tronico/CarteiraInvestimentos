...
if arquivo_upload is not None:
    ...
    if ativos.empty:
        ...
        # TABELA COMPLETA (ORDENADA POR DY 12m - DO MAIOR PARA O MENOR)
        st.subheader("📋 Visão Detalhada da Carteira (Ranking por DY)")

        df_tabela_ordenada = df_final.sort_values(by="DY 12m (%)", ascending=False, na_position="last")

        st.dataframe(
            df_tabela_ordenada.style.format({
                "Quantidade": "{:,.0f}",
                "Preço Atual (R$)": "R$ {:,.2f}",
                "Valor Total (R$)": "R$ {:,.2f}",
